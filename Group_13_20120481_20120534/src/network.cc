...
std::vector<std::vector<float> > Network::get_parameters() const {
  ...
  data.reserve(n_layer);
  for (int i = 0; i < n_layer; i++) {
    data.push_back(layers[i]->get_parameters());
  }
  return data;
}
...
std::vector<std::vector<float> > Network::get_derivatives() const {
  ...
  data.reserve(n_layer);
  for (int i = 0; i < n_layer; i++) {
    data.push_back(layers[i]->get_derivatives());
  }
  return data;
}
...
void Network::save_model(std::string file_name) 
{
  ...
  // get num layer
  int n_layer = layers.size();
  out_file.write(reinterpret_cast<char*>(&n_layer), sizeof(int));
  ...
  for (int i = 0; i < n_layer; i++) 
  {  
    std::vector<float> layer_data = layers[i]->get_parameters();

    // get layer size and write to file
    int layer_size = layer_data.size();
    out_file.write(reinterpret_cast<char*>(&layer_size), sizeof(int));

    // loop and write each parameter of layer to file
    for (int j = 0; j < layer_size; j++) 
    {
      out_file.write(reinterpret_cast<char*>(&layer_data[j]), sizeof(float));
    }
  }
}

void Network::load_model(std::string file_name) 
{
  ...
  // get num layer
  int n_layer, layer_size;
  in_file.read(reinterpret_cast<char*>(&n_layer), sizeof(int));
  data.reserve(n_layer);

  for (int i=0; i<n_layer; i++)
  {
    in_file.read(reinterpret_cast<char*>(&layer_size), sizeof(int));

    std::vector<float> layer_data;
    layer_data.reserve(layer_size);
    for (int j=0; j < layer_size; j++)
    {
      float temp = 0;
      in_file.read(reinterpret_cast<char*>(&temp), sizeof(float));
