...
__global__ void conv_basic_kernel(float *output, float *input, float * weight, float* bias, 
...
void Forward_GPU::forward_GPU_basic(float *output, float *input, float *weight, float * bias,
    ...
    int height_in, int width_in, int kernel_height)
{
    ...
    // Call the kernel
    conv_basic_kernel<<<gridSize, blockSize>>>(d_ouput, d_input, d_weight, d_bias, channel_out, channel_in, height_in, width_in, kernel_height);
